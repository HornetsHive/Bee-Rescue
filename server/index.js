...
app.post("/api/bk_insert", (req, res) => {
    ...
    const sqlINSERT = "INSERT INTO BEEKEEPERS (bk_id, fname, lname, email, phone_no, address, city, zip, pass) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    db.query(sqlINSERT, [DEFAULT, fname, lname, email, phone_no, address, city, zip, pass], (err, result) => {
        console.log(result);
    })
});
...
app.get("/api/bk_pass", (req, res) => {
    ...
    const sqlQuery = "SELECT email, pass FROM BEEKEEPERS WHERE email = ? AND pass = ?;";
...
app.get("/api/bk_get", (req, res) => {
    ...
    const sqlQuery = "SELECT * FROM BEEKEEPERS NATURAL JOIN QUALIFICATIONS WHERE email = ? AND pass = ?;";
    db.query(sqlQuery, [email, pass], (err, result) => {
        console.log(result);
    });
});
